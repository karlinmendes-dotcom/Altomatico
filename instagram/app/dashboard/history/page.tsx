...
const HistoryPage = () => {
  ...
  const historyData = useQuery(api.aiOutputs.listByUser, { createdBy: userEmail });
  const history: HISTORY[] = historyData ?? [];
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 text-center font-semibold bg-gray-200 p-2">
        <div>Template</div>
        <div className="col-span-2">Resposta da IA</div>
        <div>Data</div>
        <div>Copiar</div>
      </div>
      {history.map((item) => (
        <div key={item._id} className="grid grid-cols-1 md:grid-cols-5 gap-6 p-3 border-b">
          ...
          <div className='md:mx-14'>{new Date(item.createdAt).toLocaleDateString('pt-BR')}</div>
